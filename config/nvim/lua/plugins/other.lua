local rails_alternative_targets = {
  ...
  { context = "factories",  target = "/test/factories/%1.rb",                transformer = "pluralize" },
}
...
local rails_mappings = {
  ...
  },
  {
    pattern = "/lib/(.*).rb",
    target = {
      { context = "test", target = "/test/lib/%1_test.rb" },
...
return {
  ...
  config = config,
  keys = {
    { "<leader>o", "<cmd>:OtherVSplit<CR>", desc = "Open Other File in Split" }
  },
}
